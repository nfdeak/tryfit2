...
export function TrackerTab() {
  ...
  const adherenceData = [{ name: 'adherence', value: stats?.adherence ?? 0, fill: '#E8845A' }];

  return (
    <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
      {/* Stats row */}
      <div className="grid grid-cols-4 gap-2">
        <StatCard icon="🍽️" label="Eaten" value={String(stats?.eaten ?? 0)} />
        <StatCard icon="📊" label="Adherence" value={`${stats?.adherence ?? 0}%`} highlight />
        <StatCard icon="🔥" label="Streak" value={`${stats?.streak ?? 0}d`} />
        <StatCard icon="⏳" label="Left" value={String(stats?.remaining ?? mealsPerDay * 7)} />
      </div>
      ...
      {stats && stats.eaten > 0 && (
        <div className="bg-surface rounded-2xl p-4 flex items-center gap-4 border border-border card-glow">
          ...
          <div>
            ...
            <p className="text-dimmed text-xs font-sans mt-0.5">{stats.eaten} of {stats.total} meals tracked</p>
          </div>
        </div>
      )}
